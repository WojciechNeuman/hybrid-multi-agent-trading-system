...
The standard external baselines sharpen this point. A textbook recurrent network (GRU/LSTM), built as a minimal variation of the Mamba agent so that only the encoder differs, posted the highest \textit{validation}-window Sharpe of any learned model yet lost money out of sample --- the cleanest illustration in this study that in-window performance is not skill. A classical ARIMA model forecast no better than a random walk (directional accuracy $\approx 51\%$) and did not trade profitably. That a plain recurrent net and a linear model both fail on the same data, labels, and execution is the strongest available evidence that the heterogeneous, nonlinear, risk-managed design is doing real work rather than merely adding complexity --- and it directly answers the reasonable objection that a simpler standard model might have sufficed.
...
The deep sequence models do not yet fix every stochastic component (notably the Mamba training loop and \texttt{DataLoader} shuffling), so their walk-forward outputs vary between runs and the fund inherits this non-determinism. Full bitwise reproducibility is treated as a blocking task in the project's remediation notes.

\paragraph{Computational budget.}
Training ran on a personal laptop and a single cloud A100 GPU under modest epoch and fold budgets, so the reported results are a lower bound on what the same architectures could achieve with more compute.

\section{Practical Usability}
\label{sec:practical_usability}

Of the components, LightGBM is the easiest to operate: it trains in minutes, gives usable probabilities, and has a well-understood failure mode (probability compression toward $0.5$ in low-predictability regimes). It is not the strongest surviving agent --- Mamba and the dominance-rotation rule return more --- but the fund relies on the diversity among all streams, and its allocation layer adds little latency, being a simple causal computation over trailing returns. The principal obstacle to live deployment is not the models but the execution assumptions: moving from backtest to paper trading would expose the strategy to the real slippage, latency, and partial fills that --- as argued throughout --- ultimately matter more than headline fees.
