...
The agents span the modern time-series toolkit: gradient-boosted trees (LightGBM), a temporal convolutional network (TCN), a selective state-space model (Mamba), and parameter-free rule-based strategies. A patch-based Transformer (PatchTST), deep reinforcement learning, and genetic-programming rule search were also built but ultimately discarded for lack of out-of-sample skill. A predeclared five-agent fund is combined by a causal, capped inverse-volatility allocator.
